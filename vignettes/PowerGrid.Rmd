---
title: "Typical use of powergrid"
output:
...
# Introduction 
The powergrid package offers a number of functions that facilitate exploring the
statistical power of a study. In a typical use case (which I also show in this
vignette) you want to explore the relation between statistical power, sample
size, the effect size of interest and the assumed variability of the data. The
interrelation of these parameters is defined by the assumed distributional form
of your data, and the statistical test that you aim to perform.
...
size. You may, however use it for very different purposes. An example close to a
power analysis would be an analysis of the precision you may expect of a study
design. As a more remote application, you can think of a coverage study for
different confidence intervals. Or an exploration for planning recruitment under
various scenarios.
...
The example I chose above does not reflect the uncertainty about my study: I do
not know what SD to expect. And I may wanna explore for which effect size I may
have enough power. To explore the uncertainty about power in my study,
`powergrid` offers two plotting functions: `PowerPlot` and `GridPlot`.

### PowerPlot

First, I can plot the required
n for different effect sizes and a chosen SD to achieve a certain power.
...
Assume we have about the same situation as above, but we do not have the a
