...
parameters. In the [first situation](#Power and sample size for a t-test), a
function for calculating the power is available. In the [second
situation](#Power evaluation using simulation and resampling), the power needs
to be found through (re-)sampling. The [third example](#Target a maximum
CI97-width) concerns a situation where the objective is not a minimal power, but
a CI-width that must be at most a certain value. In the [fourth example](#Find
the upper boundary), we do not search the *lowest required* sample size, but the
*highest permissible* standard deviation to achieve a target power.
...
We see that, by setting <code>find_min = FALSE</code> Example() shows us the
...
```{R, maximum SD GridPlot, fig.width=5.5, fig.height=5.5, fig.align='center'}
