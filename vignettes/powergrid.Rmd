...
# Raw functionality in powergrid
...
## create an array with the product of three numbers
Prod = function(x, y, z){
  x * y * z
}
par = list(x = 1:10,
...
## what is, for each combination of y and z, the lowest x where the product is
## at least 20?
FindTarget(products,
           par_to_search = 'x',
           find_min = TRUE,
...
# Four use cases
I describe two basic use cases here, and two slightly more fancy once. The first
two concern the relation between power, sample size, and two further
parameters. In the first situation, a function for calculating the power is
available. In the second situation, the power needs to be found through
(re-)sampling. The third and fourth example concern situations where the
...
* You need to "slice out" one plain from your power_array. In this case, this
  ...
  by n.
* Note that in the figure, that the example is a bit above the power = 90% line. This
  is because of the resolution of parameter n: at the example value of delta,
  the arrow points at the lowest n in pars, for which the power is at least 90%.
...
The wording "target" in both the function argument and the printed result may be
...
AddExample(CI_array, slicer = list(delta = .7),
           example = list(sd = .8),
           target = .7,
           minimal_target = FALSE)
```
...
* We set "minimal_target" to FALSE, since we are looking for a CI of *at most*
  .7
...
## A special case
We have the same situation as above, where we saw that the average CI with
decreased with n, and increased with SD. Say, we have a limited number of
participants of 45 per study arm, but we can have invest a bit in our
measurement tools to have some control over SD. We may ask ourselves: what is
the highest SD at which we still find, on average, a CI width smaller than the
intended .7 points?

In this case, we may want to find for a certain n (on the x-axis) the maxmimum
sd where the width is still below .7. Note that two things are special here: 
* we look for something (CI width) that should be small, not large (like
  power). Therefore, we set `minimal_target = FALSE`
* we look for the largest acceptable SD (not the smallest n) where we can
  achieve the target value of .7. Therefore, we set `find_min = FALSE`.

```{R, special case, fig.width=5.5, fig.height=5.5, fig.align='center'}
PowerPlot(CI_array, slicer = list(delta = .7),
          target_levels = c(.6, .7, .8), # this defines the lines
          par_to_search = 'sd',
          title = "CI-width as a funtion of sd and n,\nassuming delta = .7")
...
AddExample(CI_array, slicer = list(delta = .7),
           example = list(n = 45),
           minimal_target = FALSE, # average width not larger
           target = .7, ## than .7
           find_min = FALSE)

```

We see that the highest SD for which the CI-width is still smaller than .7 is .8.

We could also get this answer in text:

```{R, special case Example, fig.width=5.5, fig.height=5.5, fig.align='center'}
CI_example = Example(CI_array,
                     example = list(delta = .7, n = 40),
                     minimal_target = FALSE, # average width not larger
                     target = .7, ## than .7
                     find_min = FALSE)
print(CI_example)
## The output of Example is a list that you can use when writing your markdown
## product.
print.default(CI_example)
```

We can also show this situation in a GridPlot:
```{R, special case GridPlot, fig.width=5.5, fig.height=5.5, fig.align='center'}
GridPlot(CI_array, target = .7, minimal_target = FALSE)
```
