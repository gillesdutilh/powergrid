...
The package (version 0.6.1) is available from
[CRAN](https://CRAN.R-project.org/package=powergrid) and may be installed
through your favorite mirror.

Note that the package is developed further here on SCTO’s github. If you want to
...
devtools::install_github("SwissClinicalTrialOrganisation/powergrid",
                         ## tag = 'v0.6.1', # replace with a specific
...
from the SCTO. Since then, many great contributions have been made by [Richard
...
* The parameter that is optimized to achieve the objective may be something you
  ...
  permissible SD).
* Two different plot function are supplied to show the relation between input
  parameters and the objective: not only showing a single sample size to achieve
  a certain power, but an illustration of the dynamics of sample size, power,
